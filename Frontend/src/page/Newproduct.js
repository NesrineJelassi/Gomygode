...
const Newproduct = () => {
  ...
  const handleSubmit = async(e)=>{
    ...
    if(name && image && category && price){
      const fetchData = await fetch(`http://localhost:5600/uploadProduct`,{
        method : "POST",
        headers : {
    ...
    <div className="p-4">
       <form className='m-auto w-full max-w-md  shadow flex flex-col p-3 bg-white' onSubmit={handleSubmit}>
        ...
        <select className='bg-slate-200 p-1 my-1' id='category' name='category' onChange={handleOnChange} value={data.category}>
          ...
          <option value={"Dosa crepe"}>Crêpes</option>
          <option value={"Pizza"}>Pizza</option>
          <option value={"Pates"}>Pâtes</option>
          <option value={"Gateau"}>Gâteaux</option>
          <option value={"Hlou Tunisien"}>Gâteaux tunisien(Hlou)</option>
        ...
        <button className='bg-red-500 hover:bg-red-600 text-white text-lg font-medium my-2 drop-shadow'>Sauvegarder</button>
       </form>
    </div>
  )
}
